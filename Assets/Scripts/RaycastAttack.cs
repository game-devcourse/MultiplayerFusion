...
public class RaycastAttack : NetworkBehaviour {
        ...
        if (attack.WasPerformedThisFrame()) {
            ...
            if (Runner.GetPhysicsScene().Raycast(ray.origin, ray.direction * shootDistance, out var hit)) {
                ...
                if(hitObject.TryGetComponent<Shield>(out var shield))
                {
                    if (shield.HasShield())
                    {
                        // Player has an active shield, handle shield logic here
                        Debug.Log("Player has a shield, no damage dealt");
                    }
                    else
                    {
                        // Player does not have a shield, deal damage
                        DealDamageToPlayer(hitObject);
                    }
                }
                else
                {
    ...
    private void DealDamageToPlayer(GameObject hitObject)
    {
